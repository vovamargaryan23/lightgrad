...
## Phase 1: The Tensor Core (Month 1)

- [ ] **N-Dimensional Tensor Class**: Build a wrapper around NumPy (initially) that supports `.data` and `.grad`.
- [ ] **Topological Sort**: Implement a robust DAG walker for the `backward()` call to handle complex graphs without recursion limits.
- [ ] **Broadcasting Engine**: Manually implement broadcasting logic for element-wise ops (Addition, Sub, Mul).
- [ ] **Matrix Multiplication (MatMul)**: Implement the derivative for $Y = A \times B$ (this is the core of all NNs).
- [ ] **Fundamental Ops**: Implement `Sum`, `Mean`, `Max`, and `Transpose` with their respective gradient formulas.

## Phase 2: The Deep Learning API (Month 2)
...
## Phase 3: Hardware & Systems Alpha (Month 3-4)
